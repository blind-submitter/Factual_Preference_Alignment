...
def _get_unique_tensor_key(tensor: torch.Tensor) -> tuple:
    ...
    if (
        ...
        and hasattr(tensor, "_local_tensor")
    ):
    ...
    try:
        storage_ptr = (
            actual_tensor.untyped_storage().data_ptr() + actual_tensor.storage_offset()
        )
...
class OffloadActivations(saved_tensors_hooks):
    ...
    def __init__(
        ...
        def verify_sufficient_virtual_memory():
            ...
            if curr_pct > self.virtual_memory_safe_pct:
                logger.warning(
                    f"{curr_pct=}% > {self.virtual_memory_safe_pct=}% of virtual memory used"
                )
        ...
        def pack_tensor(activation: torch.Tensor) -> int:
            ...
            try:
                ...
                if (
                    DTensor is not None
                    and isinstance(activation, DTensor)
                    and hasattr(activation, "_local_tensor")
                ):
            ...
            with stream_ctx:
                ...
                if has_broadcast:
                    ...
                    cpu_storage_view = torch.as_strided(
                        activation,
                        size=(storage_size // activation.element_size(),),
                        stride=(1,),
                        storage_offset=0,
                    )
        ...
        def unpack_tensor_single_stream(unpack_tensor_id: int) -> torch.Tensor:
            ...
            if modified:
                ...
                accelerator_tensor = maybe_accelerator_tensor.to(
                    self.accelerator_type, non_blocking=True
                )
    ...
    def update_model_params(self, model: nn.Module):
        ...
        for p in model.parameters():
            ...
            if (
                DTensor is not None
                and isinstance(p, DTensor)
                and hasattr(p, "_local_tensor")
            ):
