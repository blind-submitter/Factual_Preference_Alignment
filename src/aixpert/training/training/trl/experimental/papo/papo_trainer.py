...
class PAPOTrainer(GRPOTrainer):
    ...
    def _mask_image(
        self, pixel_values: torch.Tensor, mask_ratio: float = None
    ) -> torch.Tensor:
        """
        ...
        if self.mask_type == "random":
            ...
            if pixel_values.ndim == 4:  # (B, C, H, W)
                ...
                for i in range(B):
                    ...
                    # Apply mask
                    masked_pixel_values[
                        i, :, start_h : start_h + patch_h, start_w : start_w + patch_w
                    ] = 0
    ...
    def _compute_loss(self, model, inputs):
        ...
        inputs["pixel_values"] = self._mask_image(
            inputs["pixel_values"], self.mask_ratio
        )
        mask_img_per_token_logps, mask_img_entropies = (
        ...
        perception_kl = (
            ...
            - (mask_img_per_token_logps - per_token_logps)
            - 1
        )
        ...
        der_loss = (
            self.der_loss_weight1 * entropies
            + self.der_loss_weight2 * mask_img_entropies
        )
