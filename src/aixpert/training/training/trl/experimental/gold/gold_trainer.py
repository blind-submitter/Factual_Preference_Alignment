...
def build_teacher_inputs_from_texts(
    ...
    completion_token_ids = tokenizer(completion_texts, add_special_tokens=False)[
        "input_ids"
    ]
    ...
    teacher_attention_mask = pad(
        attention_masks, padding_side="right", padding_value=0
    ).bool()
...
class ULDLoss(nn.Module):
    ...
    def __init__(
        self, config: GOLDConfig, student_tokenizer=None, teacher_tokenizer=None
    ):
        super().__init__()
        ...
        self.hybrid_unmatched_weight = getattr(
            config, "uld_hybrid_unmatched_weight", None
        )
        self.beta = getattr(
            config, "beta", 1.0
        )  # For JSD loss in hybrid matched tokens
    ...
    def _compute_distillation_loss(
        ...
        student_answer_index, student_answer_size = self._get_start_and_size_answers(
            student_labels
        )
        teacher_answer_index, teacher_answer_size = self._get_start_and_size_answers(
            teacher_labels
        )
        ...
        for i in range(batch_size):
            ...
            student_probs = F.softmax(
                student_answer_logits / self.student_temperature, dim=-1
            )
            teacher_probs = F.softmax(
                teacher_answer_logits / self.teacher_temperature, dim=-1
            )
            ...
            if self.use_extended_uld:
                ...
                student_alignment_groups, teacher_alignment_groups = (
                    self._build_alignment_groups_from_ids(
            ...
            if self.use_hybrid_loss and self._vocab_mapping is not None:
                ...
                aligned_loss = self._compute_hybrid_uld_loss(
                    student_aligned, teacher_aligned
                )
            else:
                ...
                if student_vocab_size < max_vocab_size:
                    student_sorted = F.pad(
                        student_sorted, (0, max_vocab_size - student_vocab_size)
                    )
                if teacher_vocab_size < max_vocab_size:
                    teacher_sorted = F.pad(
                        teacher_sorted, (0, max_vocab_size - teacher_vocab_size)
                    )

                # Compute L1 distance (ULD approach)
                aligned_loss = F.l1_loss(
                    student_sorted, teacher_sorted, reduction="sum"
                )
    ...
    def _compute_hybrid_uld_loss(self, student_aligned, teacher_aligned):
        ...
        if self._teacher_matched_ids:
            ...
            student_matched_indices = torch.tensor(
                [self._vocab_mapping[tid.item()] for tid in teacher_matched_indices],
                dtype=torch.long,
                device=device,
            )
        else:
        ...
        teacher_matched_mask = torch.zeros(
            teacher_vocab_size, dtype=torch.bool, device=device
        )
        student_matched_mask = torch.zeros(
            student_vocab_size, dtype=torch.bool, device=device
        )
        ...
        if (
            teacher_unmatched_probs.size(-1) > 0
            and student_unmatched_probs.size(-1) > 0
        ):
        ...
        total_loss = (
            hybrid_matched_weight * matched_loss
            + hybrid_unmatched_weight * unmatched_loss
        )
...
class GOLDTrainer(SFTTrainer):
    ...
    def _prepare_dataset(
        ...
        if not is_processed or (
            self.use_uld_loss and self.teacher_tokenizer is not None
        ):
    ...
    def _prepare_dataset_with_original_text(
        ...
        with PartialState().main_process_first():
            ...
            if isinstance(
                ...
                map_kwargs["desc"] = (
                    f"Tokenizing {dataset_name} dataset (preserving original text)"
                )
            ...
            def tokenize_with_original_text(
                ...
                if "prompt" in example:  # prompt-completion case
                    ...
                    result.update(
                        {
                            ...
                            "attention_mask": [1]
                            * len(prompt_completion_ids),  # Add attention mask
                        }
                    )
                    ...
                    result["original_prompt_text"] = ""
                    result["original_completion_text"] = example.get(
                        dataset_text_field, example.get("text", "")
                    )
            ...
            if args.use_liger_kernel:
                ...
                dataset = dataset.select_columns(
                    required_columns.intersection(dataset.column_names)
                )
    ...
    def compute_loss(
        ...
        if self.use_uld_loss and self.teacher_tokenizer is not None:
            if (
                "original_prompt_text" in inputs
                and "original_completion_text" in inputs
            ):
                prompt_texts = inputs["original_prompt_text"]
                completion_texts = inputs["original_completion_text"]
                full_texts = [
                    p + c for p, c in zip(prompt_texts, completion_texts, strict=True)
                ]
            else:
                ...
                full_texts = self.processing_class.batch_decode(
                ...
                prompt_texts = self.processing_class.batch_decode(
                    inputs["prompts"], skip_special_tokens=False
                )
                completion_texts = [
                    full.replace(prompt, "", 1)
                    for full, prompt in zip(full_texts, prompt_texts, strict=True)
                ]
            ...
            shifted_student_logits = outputs_student.logits[
                :, student_prompt_length - 1 : -1, :
            ]
            shifted_teacher_logits = outputs_teacher.logits[
                :, teacher_prompt_length - 1 : -1, :
            ]
            shifted_labels = inputs["labels"][:, student_prompt_length:]
        elif self.use_liger_gkd_loss:
            # Forward only through the base models (avoid lm_head to save memory)
            unwrapped_student = self.accelerator.unwrap_model(model)
            if (
                hasattr(unwrapped_student, "get_decoder")
                and unwrapped_student.get_decoder() is not None
            ):
            ...
            if (
                hasattr(unwrapped_teacher, "get_decoder")
                and unwrapped_teacher.get_decoder() is not None
            ):
        ...
        if self.use_uld_loss:
            ...
            teacher_labels_for_loss = (
                teacher_labels if "teacher_labels" in locals() else inputs["labels"]
            )
            teacher_input_ids_for_loss = (
                teacher_input_ids
                if "teacher_input_ids" in locals()
            ...
            if (
                hasattr(self.processing_class, "pad_token_id")
                and self.processing_class.pad_token_id is not None
            ):
                student_labels[
                    student_labels == self.processing_class.pad_token_id
                ] = -100
            ...
            if (
                ...
                teacher_labels[
                    teacher_labels == self.teacher_tokenizer.pad_token_id
                ] = -100
            ...
            if hasattr(self.uld_loss_fn, "last_matched_loss") and hasattr(
                self.uld_loss_fn, "last_unmatched_loss"
            ):
                try:
    ...
    def generate_on_policy_outputs(
        ...
        if self.use_transformers_paged:
            ...
            if prompt_mask is not None:
                ...
                prompt_sequences = [
                    row.detach().cpu().tolist() for row in prompts_tensor
                ]
            generated_outputs = model.generate_batch(
                prompt_sequences, generation_config=generation_config
            )
            model.config._attn_implementation = previous_attn

            completion_ids = [
                output.generated_tokens for output in generated_outputs.values()
            ]
            generated_tokens = torch.stack(
                [torch.tensor(ids, device=model.device) for ids in completion_ids]
            )
        else:
        ...
        if prompt_mask is not None:
            ...
            prompt_lengths = (
                (inputs["prompts"] != pad_token_id).sum(dim=1).to(torch.long)
            )
        else:
    ...
    def _generate_on_policy_outputs_vllm(
        self, inputs, generation_config, pad_token_id=None
    ):
        ...
        if self.processing_class.pad_token:
            prompts_text_for_vllm = [
                p.replace(self.processing_class.pad_token, "")
                for p in prompts_text_for_vllm
            ]
        ...
        for completion_tensor in completion_ids_tensors:
            if len(completion_tensor) > max_completion_length:
                ...
                padded_completion_ids_list.append(
                    completion_tensor[:max_completion_length]
                )
