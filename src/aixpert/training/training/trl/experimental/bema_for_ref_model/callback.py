...
class CallbackHandlerWithRefModel(CallbackHandler):
    ...
    def __init__(
        self, callbacks, model, ref_model, processing_class, optimizer, lr_scheduler
    ):
            ...
            if ref_model is None:
                ...
                if hasattr(main_model, "get_base_model"):
                    ...
                    self._update_model_with_bema_weights(
                        base_model, bema_state_dict, is_peft_base=True
                    )
                else:
                    # Regular model, update directly
                    self._update_model_with_bema_weights(
                        main_model, bema_state_dict, is_peft_base=False
                    )
            else:
                ...
                if hasattr(ref_model, "get_base_model"):
                    ...
                    # Regular model, update directly
                    self._update_model_with_bema_weights(
                        ref_model, bema_state_dict, is_peft_base=False
                    )

            logger.info("BEMACallback: Updated reference model with BEMA weights")

    def _update_model_with_bema_weights(
        self, model, bema_state_dict, is_peft_base=False
    ):
