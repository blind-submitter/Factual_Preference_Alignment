...
class XPOTrainer(OnlineDPOTrainer):
    ...
    def _generate_completions(self, prompts, model):
        ...
        if self.ref_model is None:
            ...
            if is_peft_available() and isinstance(
                unwrapped_main_model_for_ref_logic, PeftModel
            ):
                actual_model_for_ref_generation = (
                    unwrapped_main_model_for_ref_logic.get_base_model()
                )
            else:
                actual_model_for_ref_generation = unwrapped_main_model_for_ref_logic
        else:
            actual_model_for_ref_generation = self.accelerator.unwrap_model(
                self.ref_model
            )

        with unwrap_model_for_generation(
            actual_model_for_ref_generation, self.accelerator
    ...
    def _compute_logprobs(self, model, model_data, ref_data, context_length):
        def compute_logprobs_for_data(m, data):
            ...
            token_logprobs = selective_log_softmax(
                logits, data["input_ids"][:, context_length:]
            )
