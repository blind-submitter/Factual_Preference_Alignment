...
class ReplayBuffer:
    ...
    def sample(self, num_samples: int) -> list[dict[str, torch.Tensor]]:
        ...
        if num_samples > len(self.heap):
            replacement = True
        chosen_indices = torch.multinomial(
            probabilities, num_samples, replacement=replacement
        ).tolist()
...
class GRPOWithReplayBufferTrainer(GRPOTrainer):
    def __init__(self, args: GRPOWithReplayBufferConfig | None = None, **kwargs):
        ...
        self.replay_buffer = (
            ReplayBuffer(args.replay_buffer_size)
    ...
    def _generate_and_score_completions(
        ...
        grouped_std_rewards = grouped_std_rewards.repeat_interleave(
            self.num_generations, dim=0
        )
    ...
    def update_replay_buffer(
        ...
        buffered_outputs = []
        for _, group_idx in enumerate(
            groups_with_variance.nonzero(as_tuple=True)[0].unique().tolist()
        ):
            group_prompt_ids, group_prompt_mask = self.slice_group_data(
                prompt_ids, prompt_mask, group_idx
            )
            ...
            for field_name, field_data in optional_fields.items():
                if field_data is not None:
                    buffered_output[field_name] = self.slice_group_data(
                        field_data, completion_mask, group_idx
                    )[0]
        ...
        if groups_with_variance.any():
            ...
            replay_buffer_scores = (group_advantages.abs() * group_std_rewards).sum(
                dim=-1
    ...
    def sample_from_replay_buffer(
        ...
        num_samples: int,
        optional_vision_fields: list[str] = None,
        optional_tensor_fields: list[str] = None,
    ) -> list[dict]:
        """
        ...
        all_optional_fields = (optional_tensor_fields or []) + (
            optional_vision_fields or []
        )
    ...
    def update_with_replay_buffer(
        ...
        groups_to_replace_idxs = (
            groups_with_variance.logical_not()
        ...
        target_completion_len = max(
            [current_batch_completion_seq_len] + sampled_completion_lengths
        )
        ...
        for field in optional_tensor_fields:
            if field in sampled_data:
                outputs_after_sampling_buffer[field] = (
                    old_per_token_logps
                    if field == "old_per_token_logps"
                    else ref_per_token_logps
                )
        ...
        if self.use_vllm and self.vllm_importance_sampling_correction:
            outputs_after_sampling_buffer["importance_sampling_ratio"] = (
                importance_sampling_ratio
            )
