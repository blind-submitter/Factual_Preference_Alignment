...
class WeightSyncWorkerExtension:
    ...
    def init_communicator(
        self, host: str, port: int, world_size: int, client_device_uuid: str
    ) -> None:
        """
        ...
        if torch.cuda.is_available() or (
            ...
            if client_device_uuid == str(
                accelerator_module.get_device_properties(self.device).uuid
            ):
                raise RuntimeError(
...
def main(script_args: ScriptArguments):
    ...
    async def generate(request: GenerateRequest):
        ...
        all_outputs = [
            output
            for output, prompts in zip(all_outputs, chunked_prompts, strict=True)
            if prompts
        ]
        ...
        logprobs: list[list[float]] = [
            [
                sanitize_logprob(next(iter(logprob.values())))
    ...
    async def chat(request: ChatRequest):
                ...
                if isinstance(message["content"], list):
                    for part in message["content"]:
                        if part["type"] == "image_pil":
                            part["image_pil"] = Image.open(
                                BytesIO(base64.b64decode(part["image_pil"]))
                            )
        ...
        all_outputs = [
            output
            for output, prompts in zip(all_outputs, chunked_messages, strict=True)
            if prompts
        ]
