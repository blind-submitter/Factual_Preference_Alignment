...
def main():
    ...
    if "--accelerate_config" in launch_args:
        ...
        if os.path.isfile(config_name):
            ...
            resources.files("trl.accelerate_configs")
            .joinpath(f"{config_name}.yaml")
            .exists()
        ):
            # Get the predefined accelerate config path from the package resources
            accelerate_config_path = resources.files("trl.accelerate_configs").joinpath(
                f"{config_name}.yaml"
            )
        else:
            raise ValueError(
    ...
    if args.command == "dpo":
        ...
        args = launch_command_parser().parse_args(
            launch_args + [str(sft_training_script)] + training_script_args
        )
        launch_command(args)  # launch training
        ...
        if (
            ...
            and script_args.data_parallel_size > 1
