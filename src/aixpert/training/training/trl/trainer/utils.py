...
def generate_model_card(
    ...
    card = ModelCard.from_template(
        card_data,
        template_path=str(
            pkg_resources.files("trl").joinpath(f"templates/{template_file}")
        ),
        base_model=base_model,
        model_name=model_name,
...
def flush_left(
    mask: torch.Tensor, *tensors: torch.Tensor
...
def flush_right(
    ...
    first_non_empty_col = (
        int(non_empty_cols.to(torch.int8).argmax()) if non_empty_cols.any() else M
    )
...
def selective_log_softmax(logits, index) -> torch.Tensor:
    ...
    ```python
    logps = torch.gather(
        logits.log_softmax(-1), dim=-1, index=index.unsqueeze(-1)
    ).squeeze(-1)
    ```
...
def shuffle_sequence_dict(
    seq_dict: dict[str, Sequence | None],
...
def split_pixel_values_by_grid(
    ...
    """
    if (
        "image_grid_thw" not in batch
        or "pixel_values" not in batch
        or "num_images" not in batch
    ):
        return batch
    ...
    sections = [
        sum(lengths[boundaries[i] : boundaries[i + 1]])
        for i in range(len(batch["num_images"]))
    ]
    split_values = list(torch.split(batch["pixel_values"], sections, dim=0))
    image_grid_thw = list(
        torch.split(batch["image_grid_thw"], batch["num_images"], dim=0)
    )
...
def get_config_model_id(config: PretrainedConfig) -> str:
    ...
    return getattr(config, "_name_or_path", "") or getattr(
        getattr(config, "text_config", None), "_name_or_path", ""
    )
