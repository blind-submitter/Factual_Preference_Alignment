    ...
    with unwrap_model_for_generation(model, accelerator) as unwrapped_model:
        for idx in range(0, len(prompts), batch_size):
            ...
            for prompt, generation in zip(
                tokenized_batch.input_ids, generations, strict=True
            ):
...
class RichProgressCallback(TrainerCallback):
    ...
    def __init__(self):
        if not is_rich_available():
            raise ImportError(
                "RichProgressCallback requires the `rich` extra. To install, run `pip install rich`."
            )
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        ...
        self.rich_group = Live(
            Panel(Group(self.training_bar, self.evaluation_bar, self.training_status))
        )
        self.rich_group.start()
        self.training_task = self.training_bar.add_task(
            "[blue]Training  ", total=state.max_steps
        )
        self.current_step = 0
    ...
    def on_prediction_step(self, args, state, control, eval_dataloader=None, **kwargs):
        ...
        if has_length(eval_dataloader):
            if self.evaluation_task is None:
                self.evaluation_task = self.evaluation_bar.add_task(
                    "[blue]Evaluation", total=len(eval_dataloader)
                )
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        ...
        for group_name, metrics in grouped_logs.items():
            table = Table(
                title=f"[bold blue]{group_name}[/]" if group_name else None,
        ...
        self.training_status.update(
            Panel(
                column_layout,
                title=f"[bold green]Step {state.global_step}[/bold green]",
                border_style="green",
            )
        )
    ...
    def on_train_begin(
        ...
        **kwargs,
        ...
        with accelerator.split_between_processes(
            ...
            completions = list(
                zip(self.ref_completions, self.ref_completions, strict=True)
            )
        ...
        if not self._weave_initialized:
            ...
            if self.model_name is None:
                self.model_name = getattr(
                    self.trainer.model_wrapped.config, "_name_or_path", "unknown_model"
                )
...
class BEMACallback(TrainerCallback):
    ...
    def _update_bema_weights(self, step: int):
        ...
        for thetat, theta0, ema, run_param in zip(
            ...
            ema.mul_(1 - beta).add_(
                thetat, alpha=beta
            )  # EMA update: ema = (1 - beta) * ema + beta * θₜ
            run_param.copy_(
                ema + alpha * (thetat - theta0)
            )  # BEMA update: run_param = ema + alpha * (θₜ - θ₀)
