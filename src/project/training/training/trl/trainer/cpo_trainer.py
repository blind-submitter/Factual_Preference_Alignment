...
class CPOTrainer(BaseTrainer):
    ...
    def cpo_loss(
        ...
        if self.alpha != 0.0:
            ...
            logits = (policy_chosen_rewards - policy_rejected_rewards).to(
        ...
        if self.alpha != 0.0:
            ...
            chosen_rewards = (
                self.beta * policy_chosen_rewards.to(self.accelerator.device).detach()
            )
            rejected_rewards = (
                self.beta * policy_rejected_rewards.to(self.accelerator.device).detach()
            )
        else:
            ...
            table = pd.DataFrame(
                ...
                data=[
                    ...
                    for prompt, pol in zip(
                        random_batch["prompt"], policy_output_decoded, strict=True
                    )
                ],
            )
