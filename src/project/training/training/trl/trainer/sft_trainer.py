...
class DataCollatorForVisionLanguageModeling(DataCollatorMixin):
    ...
    def torch_call(self, examples: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        raise KeyError(
            f"Unexpected input keys in examples: {list(examples[0].keys())}."
        )

    def _collate_language_modeling(
    ...
    def _collate_prompt_completion(
        self, examples: list[dict[str, Any]]
    ) -> dict[str, Any]:
    ...
    def __init__(
        ...
        if peft_config is not None:
            if added_tokens:
                ...
                if peft_config.trainable_token_indices is None:
                    ...
                    peft_config.trainable_token_indices["embed_tokens"].extend(
                        added_tokens
                    )

                # Ensure that the lm_head is trainable
                if (
                    peft_config.modules_to_save is None
                    or "lm_head" not in peft_config.modules_to_save
                ):
                    logger.warning(
        ...
        if peft_config is not None or (
            ...
            if model.active_adapter in model.peft_config:
                ...
                self.num_virtual_tokens = getattr(
                    peft_model_config, "num_virtual_tokens", 0
                )
        ...
        self.padding_free = args.padding_free or (
            args.packing and args.packing_strategy == "bfd"
        )
        use_flash_attention = (
            model.config._attn_implementation in FLASH_ATTENTION_VARIANTS
        )
        if self.padding_free:
        ...
        self._is_vision_dataset = (
            "image" in dataset_sample or "images" in dataset_sample
        )
        ...
        if args.loss_type == "nll":
            ...
            raise ValueError(
                f"Invalid `loss_type` {args.loss_type} passed. Supported values are 'nll' and 'dft'."
            )
        ...
        with PartialState().main_process_first():
            ...
            if not is_processed:
                ...
                def tokenize_fn(
                        ...
                        if is_conversational(example):
                            if self._is_vlm:
                                prompt = prepare_multimodal_messages(
                                    example["prompt"], images=[]
                                )
                                completion = prepare_multimodal_messages(
                                    example["completion"], images=[]
                                )
                            else:
                            ...
                            prompt_completion_ids = prompt_completion_processed[
                                "input_ids"
                            ]
                            if "assistant_masks" in prompt_completion_processed:
                                output["assistant_masks"] = prompt_completion_processed[
                                    "assistant_masks"
                                ]
                        else:
                            prompt_ids = processing_class(text=example["prompt"])[
                                "input_ids"
                            ]
                    ...
                    if (
                        "assistant_masks" in output
                        and 1 not in output["assistant_masks"]
                    ):
                        raise RuntimeError(
            ...
            if args.use_liger_kernel:
                ...
                dataset = dataset.select_columns(
                    collator_expected_keys.intersection(column_names)
                )
