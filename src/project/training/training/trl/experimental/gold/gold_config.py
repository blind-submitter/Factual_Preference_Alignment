...
class GOLDConfig(SFTConfig):
    ...
    uld_skip_student_eos: bool = field(
        default=True,
        metadata={
            "help": "Whether to skip EOS token for student in ULD loss computation."
        },
    )
    uld_skip_teacher_eos: bool = field(
        default=True,
        metadata={
            "help": "Whether to skip EOS token for teacher in ULD loss computation."
        },
    )
    ...
    vllm_server_timeout: float = field(
        default=240.0,
        metadata={
            "help": 'Timeout (in seconds) for connecting to the vLLM server when `vllm_mode="server"`.'
        },
    )
    ...
    vllm_tensor_parallel_size: int = field(
        default=1,
        metadata={
            "help": 'Tensor parallel size for the colocated vLLM engine when `vllm_mode="colocate"`.'
        },
    )
    ...
    num_completions_to_print: int = field(
        default=5, metadata={"help": "Number of completions to print."}
    )
    ...
    def __post_init__(self):
        ...
        if (
            ...
            and self.max_completion_length >= self.max_length
        ):
            raise ValueError(
            ...
            if self.uld_use_hybrid_loss:
                ...
                if self.uld_hybrid_matched_weight is not None:
                    if self.uld_hybrid_matched_weight < 0.0:
                        raise ValueError(
                            "uld_hybrid_matched_weight must be non-negative."
                        )
                    if self.uld_hybrid_unmatched_weight < 0.0:
                        raise ValueError(
                            "uld_hybrid_unmatched_weight must be non-negative."
                        )
