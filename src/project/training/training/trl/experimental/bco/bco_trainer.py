...
def _tokenize(
    ...
    answer_input_ids = [
        f[len(p) :] for f, p in zip(full_input_ids, prompt_input_ids, strict=True)
    ]
    answer_attention_mask = [
        f[len(p) :]
        for f, p in zip(full_attention_mask, prompt_attention_mask, strict=True)
    ]

    # Concat tokens to form `enc(a) + enc(a + b)[len(enc(a)):]`
    full_concat_input_ids = [
        np.concatenate([p, a])
        for p, a in zip(prompt_input_ids, answer_input_ids, strict=True)
    ]
    ...
    prompt_input_ids = [
        f[:r] for f, r in zip(full_input_ids, response_token_ids_start_idx, strict=True)
    ]
    prompt_attention_mask = [
        f[:r]
        for f, r in zip(full_attention_mask, response_token_ids_start_idx, strict=True)
    ]
    ...
    _paper = {
        ...
        "citation": textwrap.dedent("""\
            @article{jung2024binary,
                title        = {{Binary Classifier Optimization for Large Language Model Alignment}},
                author       = {Seungjae Jung and Gunsoo Han and Daniel Wontae Name and Kyoung{-}Woon On},
                year         = 2024,
