...
def clone_chat_template(
    ...
    added_tokens = [
        token
        for token in tokenizer_source.added_tokens_decoder.values()
        if token.content not in tokenizer.vocab
    ]
    ...
    ):  # for SequenceClassification models, generation_config is None
        model.generation_config.eos_token_id = tokenizer.eos_token_id
...
def prepare_fsdp(model, accelerator):
    ...
    if not (isinstance(model, FSDP) or isinstance(model, FSDPModule)):
        ...
        kwargs = {
            ...
            or fsdp_plugin.reshard_after_forward,
