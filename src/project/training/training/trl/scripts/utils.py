...
class DatasetMixtureConfig:
    ...
    streaming: bool = field(
        default=False,
        metadata={
            "help": "Whether to stream the datasets. If True, the datasets will be loaded in streaming mode."
        },
    )
...
class ScriptArguments:
    ...
    dataset_name: str | None = field(
        default=None,
        metadata={
            "help": "Path or name of the dataset to load. If `datasets` is provided, this will be ignored."
        },
    )
    ...
    dataset_train_split: str = field(
        default="train",
        metadata={
            "help": "Dataset split to use for training. If `datasets` is provided, this will be ignored."
        },
    )
    dataset_test_split: str = field(
        default="test",
        metadata={
            "help": "Dataset split to use for evaluation. If `datasets` is provided, this will be ignored."
        },
    )
    ...
    gradient_checkpointing_use_reentrant: bool = field(
        default=False,
        metadata={
            "help": "Whether to apply `use_reentrant` for gradient checkpointing."
        },
    )
...
def init_zero_verbose():
    ...
    logging.basicConfig(
        format=FORMAT, datefmt="[%X]", handlers=[handler], level=logging.ERROR
    )
...
class TrlParser(HfArgumentParser):
    ...
    def parse_args_and_config(
        ...
        if fail_with_unknown_args and config_remaining_strings:
            raise ValueError(
...
def get_dataset(mixture_config: DatasetMixtureConfig) -> DatasetDict:
    ...
    for dataset_config in mixture_config.datasets:
        logger.info(
            f"Loading dataset for mixture: {dataset_config.path} (config name: {dataset_config.name})"
        )
    ...
    if datasets_list:
        ...
        if isinstance(
            combined_dataset, datasets.Dataset
        ...
        if mixture_config.test_split_size is not None:
            logger.info(
                f"Splitting dataset into train and test sets with test size: {mixture_config.test_split_size}"
            )
            combined_dataset = combined_dataset.train_test_split(
                test_size=mixture_config.test_split_size
            )
