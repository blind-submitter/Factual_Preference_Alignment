...
class DataCollatorForPreference(DataCollatorMixin):
    ...
    def torch_call(
        ...
        # ---- factuality scores ----
        if "h_w" in examples[0]:
            h_w = torch.tensor(
                [float(ex["h_w"]) for ex in examples], dtype=torch.float32
            )

        if "h_l" in examples[0]:
            h_l = torch.tensor(
                [float(ex["h_l"]) for ex in examples], dtype=torch.float32
            )
...
class FactualDPOTrainer(BaseTrainer):
    """
    ...
    def __init__(
        ...
        if isinstance(ref_model, str):
            ref_model = create_model_from_path(
                ref_model, **args.ref_model_init_kwargs or {}
            )
        elif args.ref_model_init_kwargs is not None:
            logger.warning(
        ...
        self.loss_type = (
            args.loss_type if isinstance(args.loss_type, list) else [args.loss_type]
        )
    ...
    def dpo_loss(
        ...
        if self.f_divergence_type == FDivergenceType.ALPHA_DIVERGENCE:
            ...
            # ============================
            # SAFE-DPO MODIFICATION START
            # ============================

            # Only apply if safety labels exist in the batch
            if "h_w" in model_output and "h_l" in model_output:
                h_w = model_output["h_w"].float().to(logits.device)
                h_l = model_output["h_l"].float().to(logits.device)
                Delta = self.args.delta

                # Debug only first few steps to avoid spam
                if self.state.global_step < 5:
                    print("\n[SafeDPO DEBUG]")
                    print("Delta:", Delta)
                    print("h_w[:10]:", h_w[:10])
                    print("h_l[:10]:", h_l[:10])
                    print("(h_l - h_w)[:10]:", (h_l - h_w)[:10])
                    print("logits BEFORE:", logits[:10])

                # SafeDPO modifies the margin BEFORE the sigmoid
                logits = logits - (h_l - h_w) * Delta

                # ============================
                # SAFE-DPO MODIFICATION END
                # ===========================
    ...
    def get_batch_loss_metrics(
        ...
        if self.args.use_liger_kernel:
            ...
            if "h_w" in batch:
                model_output["h_w"] = batch["h_w"]
            if "h_l" in batch:
                model_output["h_l"] = batch["h_l"]
